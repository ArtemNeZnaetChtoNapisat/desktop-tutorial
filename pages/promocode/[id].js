     ...
     <div className="block_cartochka_content">
        ...
        <div className="content_title">
          ...
          <div className="image">
            ...
            <Image src={items['image']} alt="Изображение магазина" className="image_dec" width={300} height={300} style={{width:"100%", height:"auto"}}/>
            <Image src={items['image']} alt="Изображение магазина" className="image_mob" width={150} height={150} style={{width:"100%", height:"auto"}}/>
        </div>
          </div>
       ...
       { !items['promocode']=="" ? <button className="button_copycode" onClick={()=>buttonCopy(items['promocode'])}>{items['promocode']} <Icon_copy/></button> : null}
        ...
        <div className="user_button">  <Link href={user}><div className="user_button_adaptive"><div className="avatar_block"><Image placeholde="blur" src={items['user_image']} width={40} height={40} style={{width: "100%", height:"auto",}}/></div><p>{items['nickname']} {Number(items['verefy']) ? <Svg_verefy/> : null}</p>  </div></Link> </div> </div> 
...
console.log(params)
if(params.id!=="test"){
  const response = await fetch("https://kotedze.ru/server/post/?post="+params.id)
