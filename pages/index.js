...
import { Pagination, Content, No_content} from "../UI/ui";
...
function Home({result}) {
    ...
    function Shop_scroll(){
        ...
        <>
        <Link href={to}>
        <div className="shop_cartochka">
            {/* <img className="image_shop_cartochka" src={props.img} title={props.title}/> */}
            <Image className="image_shop_cartochka" src={props.img} title={props.title} width={100} height={10} style={{width: '100px', height: 'auto',}}/>
        </div>
        </Link>
        </>
            );
        ...
        function Cartochka_shop_all(){
         ...
         <Link href={to}>
         <div className="shop_cartochka">
             <div className="container_shop_all">
                <div className="image_shop_all">
             </div>
             <p className="title_image_shop_all">Смотреть все магазины</p>
             </div>
         </div>
         </Link>
         </>
        ...
        function click_right(i){
           ...
           //et scroll = test.scrollLeft;
            ...
            return (<>
        <div className="block_shop" id="test">
            <button className="button_cartochka_left" onClick={click_left}></button>
        <div  className="shop_cartochka_block" >
        <Shop_cartochka img="https://kotedze.ru/image/shop/litres.png" shop="litres" title="Литрес"/>
        <Shop_cartochka img="https://kotedze.ru/image/shop/megamarket.png" shop="megamarket" title="Мегамаркет"/>
        <Shop_cartochka img="https://kotedze.ru/image/shop/y-market.png" shop="yandex-market" title="Яндекс Маркет"/>
        <Shop_cartochka img="https://kotedze.ru/image/shop/aliexpress.png" shop="aliexpress" title="Алиэкспресс"/>
        <Shop_cartochka img="https://kotedze.ru/image/shop/ozon.png" shop="ozon" title="Озон"/>
        <Shop_cartochka img="https://kotedze.ru/image/shop/m-video.png" shop="m-video" title="М.Видео"/>
        <Shop_cartochka img="https://kotedze.ru/image/shop/samokat.png" shop="samokat" title="Самокат"/>
        <Cartochka_shop_all/>
        
        </div>
        <button className="button_cartochka_right" onClick={click_right}></button>
        </div>
        </>);
        }
...
export async function getServerSideProps(context) {//getStaticProps(context){  
    ...
    console.log(page)
      const response = await fetch("https://kotedze.ru/server/post/?page="+page)
